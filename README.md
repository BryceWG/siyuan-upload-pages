...
Let a document fly. PaperKite turns a SiYuan document into a self-contained static page and hosts it in your own Cloudflare Pages or Vercel project.
...
All you need are credentials. The project does not have to exist beforehand: fill in the credentials and a project name in the plugin settings, then use "Check project" → "Check / create" — the project is created under that name when it is missing, and only read when it already exists.

### Cloudflare Pages

- **API Token**: scoped `Account` → `Cloudflare Pages` → `Edit`
- **Account ID**: detected automatically when left empty; fill it in if the token cannot read the account
- **Project name** and **deployment branch** (`main` by default)
- The plugin creates a **Direct Upload** project. An existing project can be reused only if it is Direct Upload too — Git-integrated projects cannot be uploaded to from here, and the type cannot be changed after creation
...
- **Access Token**: created under account settings → Tokens, scoped to the account or team that owns the project
- **Project name** and **deployment target** (production / preview); team projects also need the **Team ID**
- The project the plugin creates is not connected to Git. Do not connect an existing project to Git either, or the two sources will overwrite each other
...
The click first opens the "Export options" dialog; the publish starts once the options are confirmed. The deployment URL is copied to the clipboard when the publish succeeds.
