...
弹窗中的选项每次发布都可以单独调整，确认后的取值会作为下一次的默认值（保存在 `data/storage/petal/siyuan-upload-pages/publish-template.json`）：

- **在页面顶部显示文档标题**
- **正文最大宽度**：CSS 长度值，例如 `800px`
- **包含正文中引用的其他文档**：正文里的块引用、文档链接和嵌入块所指向的文档，会作为章节追加到页面末尾，原来的引用变成页内锚点跳转。只展开一层——被包含文档内部的引用仍是纯文本
- **在右侧显示目录导航**：纯静态锚点目录，窄屏（≤1100px）时移到正文上方
- **目录中包含被引用的文档**：需要同时开启上面两项，被包含文档的标题及其小节会一起进入目录
